...
router.get('/:id', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'SELECT * FROM Shipment WHERE ShipmentID = $1', [id]
    );
...
router.post('/', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      ...
      [shipmentid, goodid, portid, traderid, shipmentdate, direction.toUpperCase(), status.toUpperCase()]
    );
  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE Shipment
       ...
       WHERE ShipmentID = $7
       RETURNING *`,
      [goodid, portid, traderid, shipmentdate, direction?.toUpperCase(), status?.toUpperCase(), id]
    );
...
router.delete('/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const result = await pool.query(
      'DELETE FROM Shipment WHERE ShipmentID = $1 RETURNING *', [id]
    );
    if (result.rows.length === 0)
      return res.status(404).json({ error: 'Shipment not found' });
